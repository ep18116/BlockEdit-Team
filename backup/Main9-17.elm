...
viewPallet model (ASTxy (x, y) (ASTne n b r)) = 
    ...
    [ ("N", brickSvg n model.getBrickSize ) -- 実際のブロックの描画はbrickSvgで
        --,("D", button [ onClick MsgDelete ] [ text "-" ])
    ]  
...
viewASTxy model (ASTxy (x, y) (ASTne n b r)) = 
    ...
    [ ("N", brickSvg n model.getBrickSize ) -- 実際のブロックの描画はbrickSvgで
    , ("R", lazy4 viewAST model True (x + interval model, y ) r)
    , ("B", lazy4 viewAST model False (x, y + interval model ) b)
    ]

-- 根以外の描画
viewAST: Model -> Bool -> Position -> AST Node -> Html Msg
viewAST model isRight (x,y) ast = 
    case ast of
        Nil ->
            div [] [] 
        AST node b r ->
            Keyed.node "div"
                [ style "position" "absolute"
                , style "top"  <| (if isRight then "0" else String.fromFloat ( model.getBrickSize * 0.9)) ++ "px"
                , style "left" <| (if isRight then String.fromFloat (model.getBrickSize * 0.9) else "0") ++ "px"
                ...
                , preventDefaultOn "contextmenu" 
                      <| whenNotDragging model
                             <| whenRightButtonIsDown
                                    <| Decode.succeed
                                           <| MsgCloneUs (ASTxy ( x, y ) (ASTne node b r))
                ]
                [ --("N", brickSvg size) 変更箇所
                  ("N", brickSvg node model.getBrickSize ) -- 実際のブロックの描画はbrickSvgで
                , ("R", lazy4 viewAST model True  (x +  interval model, y ) r)
                , ("B", lazy4 viewAST model False ( x, y +  interval model ) b)
                ]                        
